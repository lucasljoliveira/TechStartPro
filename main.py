...
# CATEGORIES!
    ...
    def selectProduct(self, search, searchType):
        ...
        try:
            ...
            c = conn.cursor()
            if   searchType == 1:
                query = "SELECT * FROM PRODUCT WHERE NAME LIKE '%' || :p_stype || '%' OR :p_stype = ''"
            elif searchType == 2:
                query = "SELECT * FROM PRODUCT WHERE DESCRIPTION LIKE '%' || :p_stype || '%' OR :p_stype = ''"
            elif searchType == 3:
                query = "SELECT * FROM PRODUCT WHERE PRICE = :p_stype OR :p_stype = ''"
            elif searchType == 4:
                query = "SELECT PRODUCT.* FROM PRODUCTCATEGORY INNER JOIN PRODUCT ON PRODUCTCATEGORY.IDPRODUCT = PRODUCT.IDPRODUCT INNER JOIN CATEGORY ON CATEGORY.IDCATEGORY = PRODUCTCATEGORY.IDCATEGORY WHERE CATEGORY.NAME LIKE '%' || :p_stype || '%' OR :p_stype = ''"
            if   searchType == 0:
                query = "SELECT * FROM PRODUCT WHERE IDPRODUCT = :p_stype OR :p_stype = ''"
...
def main(ui):
    ...
    def deleteProduct():
        ...
        if option == QMessageBox.Yes:
            if ui.tableWidget_products.selectionModel().hasSelection():
                p = db.deleteProduct(db.selectProduct(ui.tableWidget_products.item(ui.tableWidget_products.selectionModel().currentIndex().row(), 0).text(), 0)[0])
                searchProduct()

    def searchProduct():
        products = db.selectProduct(ui.lineEdit_Search.text(),ui.comboBox_searchType.currentIndex())
        if products != []:
            ...
            msg.setText('No registered products.')
            msg.setStandardButtons(QMessageBox.Ok)
            option = msg.exec_()
